...
import { SIGNAL, fetchLive } from '../src/live-adapter.js';
...
const { store, state } = upsertReading(before, result.reading, result.excerpt);
...
console.log(`  대상      ${result.targetDate} 하루(KST)에 생성된 공개 저장소`);
console.log(`  집계      ${result.incomplete ? '불완전(incomplete_results=true)' : '완전'} · 남은 호출 ${result.rateRemaining ?? '?'}회`);
