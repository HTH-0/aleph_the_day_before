...
| 구획 | 내용 | 근거 |
|---|---|---|
| **CURRENT** · 현재 상태 | 값·단위·출처·출처 관측 시각·조회 시각·기준 시간대, 신선도 태그, 실패 안내 | C04~C09, C18 |
| **HISTORY** · 일별 추이 및 대조 | KST 날짜별 값·단위·어제 대비·출처 관측 시각·출처 URL | C23·C24의 화면 표시값 |
| **FALLBACK** · 예외 상황 재현 | (아코디언) fixture 재생, 실패 5종 안내와 다음 행동, 다시 시도, 합성 상태 | C12~C19 |
...
| **무엇을** | ① `지금 다시 조회`를 누른다. ② `FALLBACK` 구획을 펼치고 `느린 응답`을 누른다. ③ 실패 상자의 `다시 시도`를 누른다. |
...
| C03~C09 | `CURRENT` — 값·단위·출처·출처 관측 시각·조회 시각·기준 시간대 |
| C10 | `CURRENT`의 표시값 = `data/daily.json` 저장값 = 응답의 `total_count`. 개발자 도구 Network 탭에서 대조 가능 |
