...
오늘의 진짜 정보판 — 어제 생긴 공개 GitHub 저장소
...
| 구획 | 내용 | 근거 |
|---|---|---|
| 헤드라인 | 뷰포트 비례로 확대되는 큰 값(clamp 84~208px)·단위·어제 대비, 신선도 태그, 그 아래 한 줄 메타(출처 · 관측 시각 · 조회 시각 · Asia/Seoul) | C04~C09, C18 |
| 통계 카드 4개 | LATEST·HIGH·LOW·TRACKED — 보존된 기록에서 매번 다시 계산 | — (표시 요구 없음, 값 확인 보조) |
| 추이 차트 | 영역 채우기 + 5줄 격자 + 점별 값 라벨 | — (표시 요구 없음, 값 확인 보조) |
| **HISTORY** | KST 날짜별 값·단위·어제 대비·출처 관측 시각·출처 URL | C23·C24의 화면 표시값 |
| **FALLBACK** | (아코디언) 재생 타임라인, 실패 5종 안내와 다음 행동, 다시 시도, 합성 상태 | C12~C19 |

조건이 요구한 여섯 항목(값·단위·출처·출처 관측 시각·조회 시각·기준 시간대)은 접지 않고 항상 보이게 두었습니다. 조건에 없는 집계 범위·호출 주소·완전성 플래그만 `상세` 토글 안으로 옮겼습니다.
...
| **무엇을** | ① 큰 숫자와 그 아래 메타 한 줄을 확인한다. ② `FALLBACK`을 펼치고 `느린 응답`을 누른다. ③ 실패 상자의 `다시 시도`를 누른다. |
...
| C03~C09 | 헤드라인 — 값·단위·출처·출처 관측 시각·조회 시각·기준 시간대 |
| C10 | 헤드라인 표시값 = `data/daily.json` 저장값 = 응답의 `total_count`. 개발자 도구 Network 탭에서 대조 가능 |
| C11 | 호출 주소에 키 파라미터 없음. `git grep -iE "api[_-]?key\|secret\|token"` 결과 없음 |
| C12~C16 | `FALLBACK` — 실패 5종 각각 다른 `error_code`와 다른 안내·다음 행동 |
...
| C22~C24 | `HISTORY` — `data/daily.json`의 실제 KST 날짜 2건과 어제 대비 값 |
