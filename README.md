...
| 02 시간대 분포 | 6시간 구간 4개 막대와 구간 합계 대조 (저장된 파일에서 읽습니다) |
| 03 대조표 | 원자료 · 저장값 · 화면값 6행, 행마다 일치 판정 (화면값은 DOM 에서 실제로 읽습니다) |
| 04 보존 기록 | `data/daily.json` 표와 뺄셈식 재계산 패널 |
| 05 장애 재생 | fixture 9개 버튼, 실패 5종이 서로 다른 문장·다음 행동, 다시 시도 → 회복 |
| 06 자가검증 | 공개 꾸러미 해시 대조 + 재생 순서 7가지를 브라우저에서 다시 실행 |
| 07 확인 방법 | 4줄 확인법과 지키는 규칙 |

## 하루 안의 6시간 구간 분포

하루 합계가 어느 시간대에서 나왔는지 보기 위해, Actions 가 하루 합계를 조회한 뒤 같은 검색 조건에
시각 범위만 좁힌 조회를 네 번 더 합니다 (`00–06` / `06–12` / `12–18` / `18–24`, 모두 KST).

- 저장되는 값 `normalized_value` 는 **언제나 하루 전체 구간 조회 한 건**에서 나옵니다.
  구간 네 개는 그 숫자의 구성만 보여 주는 딸린 정보이며 일별 행 수를 늘리지 않습니다.
- 화면은 구간을 직접 조회하지 않고 저장된 파일에서 읽습니다.
  덕분에 심사자가 아무리 눌러도 출처 호출 수는 구간 기능 도입 전과 같습니다.
- 구간 하나라도 실패하면 그 구간은 비워 두고 합계 대조를 하지 않습니다. 추정해 채우지 않습니다.
- 구간 합계와 하루 합계가 어긋나면 (검색 인덱스가 조회 사이에 갱신되는 경우) 숨기지 않고 차이를 그대로 표시합니다.
...
- 6시간 구간은 일별 기록의 키를 바꾸지 않습니다. 하루는 언제나 한 행입니다.
