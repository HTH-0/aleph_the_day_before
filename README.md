...
- 신호: 서울시청(37.5665, 126.9780) 지상 2m 기온
- 출처: Open-Meteo 공개 기상 API (키 없음, CORS 허용)
- 기준 시간대: `Asia/Seoul`
...
| **무엇을** | ① `지금 다시 조회`를 누른다. ② 04번 칸에서 `느린 응답`을 누른다. ③ 이어서 `다시 시도`를 누른다. |
| **통과** | ①에서 값·단위·출처·출처 시각·조회 시각·`Asia/Seoul`이 한 화면에 있고 02번 대조표가 전부 `일치`. ②에서 `stale / timeout`, 마지막 정상값 105가 오래된 값 표시로 남고 행 1건. ③ 뒤 `fresh / none`, 행 2건, 새 합성 날짜 `2026-08-25` 행 정확히 1건, 값 120, 변화 +15. |
| **안 될 때** | 큰 숫자 자리에 마지막 정상값이 남고 노란 안내 상자가 뜬다. 실패 다섯 종류가 서로 다른 문장과 서로 다른 다음 행동을 보여 준다. 03번의 보존된 기록은 실패해도 줄지 않는다. |
...
| C03~C09 | 화면 01번 칸 — 값·단위·출처·출처 관측 시각·조회 시각·기준 시간대 |
| C10 | 화면 02번 칸 — 원자료 / 저장값 / 화면값 3열 대조 |
| C11 | 호출 주소에 키 파라미터 없음. `git grep -iE "api[_-]?key\|secret\|token"` 결과 없음 |
| C12~C16 | 화면 04번 칸 — 실패 5종 각각 다른 `error_code`와 다른 안내 |
| C17·C18 | 실패 뒤에도 행 1건과 마지막 정상값 105 유지, `오래된 값(stale)` 표시 |
| C19 | 실패 상자의 `다시 시도` → `T04-RECOVER-D2` → `fresh/none`, 행 2건, 신규 행 1건 |
| C20·C21 | `record_id = signal_id:record_date` 단일 키 upsert. 04번 표의 `갱신` 열, 05번 자가검증 |
| C22~C24 | 화면 03번 칸 — `data/daily.json`의 실제 KST 날짜 2건과 재계산 식 |
| C25 | 좌표는 서울시청. 방문자 정보 수집·저장 없음. 쿠키·로컬스토리지 미사용 |
| C26 | 실패 재생 입력은 전부 `vendor/aleph-t04/fixtures/*.json`. 05번 칸에서 해시 대조 |
